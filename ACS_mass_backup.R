...
foreach(ii = seq_along(s)) %dopar% {
  ...
  tryCatch({
    mapply(function(y, v) {
      ...
      write_parquet(x, paste0("/Volumes/T7 Shield/Census_ACS_Backup/DP05",
                              "_", s[ii], "_tracts_", y, ".parquet"))
      write.csv(v, paste0("/Volumes/T7 Shield/Census_ACS_Backup/DP05",
                          "_", s[ii], "_tracts_", y, "_vars.csv"))
    },
    y = yrs,
    v = vars.dp05,
    SIMPLIFY = FALSE)
